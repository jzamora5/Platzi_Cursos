...
def agregar_curso(json):
    ...
    operator = {'$addToSet': {'cursos': curso}}
    return str(db.carreras.update_one({'_id': ObjectId(json['id_carrera'])}, operator).modified_count)


def borrar_curso_de_carrera(json):
    operator = {'$pull': {'cursos': {'_id': ObjectId(json['id_curso'])}}}
    return str(db.carreras.update_one({'_id': ObjectId(json['id_carrera'])}, operator).modified_count)
